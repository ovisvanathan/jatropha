/*
 * $Id: AbstractSearchable.java,v 1.3 2005/12/05 16:51:32 kizune Exp $
 *
...
public abstract class AbstractSearchable implements Searchable {
 ...
 //   protected SearchResult lastSearchResult = new SearchResult();
    ...
    public int search(String searchString, int startIndex, boolean backward) {
        ...
        int matchingRow = doSearch(pattern, startIndex, backward);
        moveMatchMarker();
        return matchingRow;
     ...
     *   no match found.
     */
    protected int doSearch(String pattern, final int startIndex, boolean backwards) {

		System.out.println("in dosearch pattern = " + pattern);
		System.out.println(" startIndex = " + startIndex);
		System.out.println(" backwards = " + backwards);
		
		
        if (isTrivialNoMatch(pattern, startIndex)) {
   			System.out.println("in if istriv ");
        ...
        if (isEqualStartIndex(startIndex)) { // implies: the last found coordinates are valid
   			...
   			System.out.println("in if isequalstart ");

        
            if (!isEqualPattern(pattern)) {
            	
  	   			System.out.println(" pattrerns not equal ");
            	
               SearchResult searchResult = findExtendedMatch(pattern, startIndex);
               if (searchResult != null) {
                   updateState(searchResult);
	  	   			System.out.println(" pattrerns not equal - ret ");                   
                   return lastSearchResult.foundRow;
               }
            ...
            // for looking for the next/previous match
  			...
  			System.out.println(" calling movestartpos ");                   
            ...
            // and make sure a -1 startIndex is mapped to first/last row, respectively.
   			...
   			System.out.println(" in else calling  adjustStartPosition ");                   
            startRow = adjustStartPosition(startIndex, backwards); 
        }
        
        System.out.println(" calling  findMatchAndUpdateState ");                   
     ...
     * @param backwards <code>true</code> if we should perform search towards the beginning
     */
    protected abstract void findMatchAndUpdateState(String pattern, int startRow, boolean backwards);
     ...
     */
    protected boolean isTrivialNoMatch(String pattern, final int startIndex) {
        ...
        if (startIndex < 0) {
            if (backwards) {
                return getSize() - 1;
                    ...
                    return (lastSearchResult == null)? 0 : lastSearchResult.foundRow;
            }
        }
        return startIndex;
    }
     ...
     */
    protected boolean isEqualPattern(String pattern) {
        return pattern.equals(lastSearchResult.getRegEx());
    }
     ...
     */
    protected abstract SearchResult findExtendedMatch(String pattern, int row);


    public String getSearchPattern() {
    	return lastSearchResult.pattern;
    } 
    ...
    public int getSearchResultColumn() {
    	return lastSearchResult.foundColumn;
    } 
   ...
   protected abstract int getSize();
    ...
    public static class SearchResult {
        ...
        public SearchResult(String ex, MatchResult result, int row, int column) {
            pattern = ex;
            matchResult = result;
            foundRow = row;
            foundColumn = column;
