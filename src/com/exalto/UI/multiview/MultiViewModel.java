...
//    private Map nestedPerspectiveComponents; //key=description, value mull or perspectiveComponent
    ...
    public MultiViewModel(MultiViewDescription[] descs, MultiViewDescription defaultDescr, 
...
//        nestedPerspectiveComponents = new HashMap();
        ...
        for (int i = 0; i < descriptions.length; i++) {
            ...
            MultiViewPerspective mperspec = new MultiViewPerspective(descriptions[i]);
            
   //         nestedPerspectives.put(descriptions[i], Accessor.DEFAULT.createPerspective(descriptions[i]));
            nestedPerspectives.put(descriptions[i], mperspec);
            if (element != null) {
                // set the observer..
...
//                nestedPerspectiveComponents.put(descriptions[i], Accessor.DEFAULT.createPersComponent(element));
  ...
  //  	  System.out.println(" currentEditor = " + currentEditor.getDisplayName());
  //  	  System.out.println(" new description = " + description.getDisplayName());
 ...
 // 	    System.out.println(" act currentEditor = " + currentEditor.getDisplayName());
...
//    }
    ...
    public MultiViewElement getElementForDescription(MultiViewDescription description) {
     ...
     */
     public synchronized MultiViewElement getElementForDescription(MultiViewDescription description, boolean create) {
   /* 	 
 ...
 //           nestedPerspectiveComponents.put(description, Accessor.DEFAULT.createPersComponent(element));
    ...
    */    
    	MultiViewElement element = description.getElement();
        return element;
    }
...
/*
  ...
  */      
    ...
    void fireActivateCurrent(MouseEvent e) {
        if (listeners != null) {
            synchronized (listeners) {
                Iterator it = listeners.iterator(); 
                while (it.hasNext()) {
                    ElementSelectionListener list = (ElementSelectionListener)it.next();
                    list.selectionActivatedByButton(e);
                }
            }
        }
    }

    void fireActivateCurrent(ActionEvent e) {
        if (listeners != null) {
            synchronized (listeners) {
                Iterator it = listeners.iterator(); 
                while (it.hasNext()) {
    ...
    private class BtnGroup extends ButtonGroup {
       ...
       //         System.out.println(" active = " + desc.getDisplayName());
